...
class ForgotPwdViewModel @Inject constructor(
    ...
    fun validateCredentials(
        ...
        passwordError = when {
            ...
            !newPassword.any { it.isDigit() } -> context.getString(R.string.password_must_contain_at_least_1_number)
            else -> null
        }
    ...
    fun forgotPassword(context: Context, newPassword: String) {
        viewModelScope.launch {
            ...
            repo.forgotPassword(ForgotPwdRequest(token, newPassword)).handle(
                onSuccess = { data ->
